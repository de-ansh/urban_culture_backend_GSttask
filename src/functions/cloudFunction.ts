...
export const createEInvoice = async (req: Request, res: Response): Promise<any> => {
    try {
        ...
        if (!totalAmount) {
            return res.status(400).json({ message: "Total booking amount is missing" });
        }
        ...
        if (!userData) return res.status(500).json({ message: "User data is undefined" });

        // Fetch company (service provider) details
        const companyDoc = await db.collection("companies").doc(userData.companyId).get();
        if (!companyDoc.exists) return res.status(404).json({ message: "Company not found" });


        const companyData = companyDoc.data();
        
        if (!companyData) return res.status(500).json({ message: "Company data is undefined" });
        ...
        return res.status(200).json({ message: "GST data created and next API triggered successfully", gstData });
